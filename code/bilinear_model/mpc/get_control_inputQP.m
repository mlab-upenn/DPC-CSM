function u = get_control_inputQP(x0, bm, sim, v_now, vPred, ymin, ymax, ucost, umin, umax, xref)
    % Set parameters
    ...
    BB = [];
    for ii = 1:N_mpc % columns
        AiB = zeros(12*(ii-1),4);
        Bu_tmp = bm.Bu + Bvu_aux{ii} + Bxu_aux{ii};
        for jj = ii:N_mpc % rows
            AiB = [AiB; bm.A^(jj-ii) * Bu_tmp];
        end
        BB = [BB AiB];
    end
    ...
    Bd = bm.Bv;
    AiBd=Bd;
    BBd=kron(eye(N_mpc),AiBd);
    for i=1:N_mpc-1;
        AiBd=bm.A*AiBd;
        BBd=BBd+kron(diag(ones(N_mpc-i,1),-i),AiBd);
    end
    BBd = BBd*reshape(vPred',[N_mpc*8,1]);

    % objective function
    Q = diag([10^3,zeros(1,nx-1)]);
    QQ=blkdiag(kron(eye(N_mpc),Q));
    R = eye(nu);
    RR=kron(eye(N_mpc),R);

    H=2*(RR + BB'*QQ*BB);
    H=(H+H')/2; %make symmetric (to avoid warnings in quadprog)
    H = blkdiag(H,zeros(nslack,nslack));
    F=2*AA'*QQ*BB;
    f=F'*x0;
    xref = repmat(xref,[N_mpc,1]);
    Fref = -2*xref'*QQ*BB;
    Fd=2*BBd'*QQ*BB;
    f=f+Fd'+Fref'; 
    f = [f; 10^3*ones(nslack,1)];
    %% Optimization problem
    
    [ u_opt, fmin, status, details ] = cplexqp(H, f', A_ineq, b_ineq);
